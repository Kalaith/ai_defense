...
impl GameplayState {
    ...
    fn requirement_label(&self, req: &str) -> String {
        ...
        if self.map_state.buildings.iter().any(|b| b.id == req) {
            return self.building_display_name_by_id(req);
        }
    ...
    pub fn draw_intro(&mut self) {
        ...
        let cx = px + pw * 0.5;
        draw_centered_ui_text("LAST ASSEMBLY", cx, py + 46.0, 28.0, dark::NEGATIVE);
        draw_centered_ui_text(
    ...
    pub fn draw_pause_menu(&mut self) {
        ...
        macroquad_toolkit::ui::draw_surface(Rect::new(px, py, pw, ph), &surface);
        draw_centered_ui_text("PAUSED", px + pw * 0.5, py + 44.0, 26.0, dark::ACCENT);
    ...
    pub(crate) fn update_coach(&mut self) {
        ...
        if done {
            self.coach.step += 1;
            if self.coach.step >= super::COACH_STEPS.len() {
                self.finish_coach();
            }
        }
    }
    ...
    pub fn draw_coach(&mut self) {
        ...
        let Some((title, body)) = super::COACH_STEPS.get(self.coach.step).copied() else {
            return;
        };
        ...
        draw_panel_text(body, bx + 12.0, by + 59.0, bw - 24.0, 11.0, dark::TEXT_DIM);
    ...
    pub fn draw_salvage_report(&mut self) {
        ...
        draw_centered_ui_text(&title, cx, panel_y + 40.0, 24.0, dark::POSITIVE);
        draw_centered_ui_text(
        ...
        let lines = [
            ...
            format!(
                ...
                report.scavengers_sent, report.scavengers_returned, lost_pop
            ),
    ...
    pub(crate) fn start_beacon(&mut self) {
        ...
        self.cycle_baseline = super::CycleBaseline {
